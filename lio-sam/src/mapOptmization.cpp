...
public:
    ...
    */
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        pcl::PointCloud<PointType>::Ptr prevKeyframeCloud(new pcl::PointCloud<PointType>());
        {
            ...
            // 提取闭环匹配关键帧前后相邻若干帧的关键帧特征点集合，降采样，也就是构成一个帧到局部地图的一个匹配问题
            loopFindNearKeyframes(prevKeyframeCloud, loopKeyPre, historyKeyframeSearchNum);
             // 如果特征点较少，返回
            if (cureKeyframeCloud->size() < 300 || prevKeyframeCloud->size() < 1000)
                return;
            if (pubHistoryKeyFrames.getNumSubscribers() != 0)
                // 把局部地图发布出去供rviz可视化使用，闭环匹配关键帧局部map
        ...
        icp.setMaxCorrespondenceDistance(historyKeyframeSearchRadius*2);//最大的相关距离
        icp.setMaximumIterations(100);
        icp.setTransformationEpsilon(1e-6); //变化步长
        icp.setEuclideanFitnessEpsilon(1e-6);
        icp.setRANSACIterations(0);

        // 设置两个点云，scan-to-map，调用icp匹配
        icp.setInputSource(cureKeyframeCloud);
        icp.setInputTarget(prevKeyframeCloud);
        pcl::PointCloud<PointType>::Ptr unused_result(new pcl::PointCloud<PointType>());
        // 执行点云配准
        icp.align(*unused_result);
        // 检查icp是否收敛且得分是否满足要求，未收敛，或者匹配不够好
        if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore)
            return;

        // 把修正后的当前点云发布供可视化使用，发布当前关键帧经过闭环优化后的位姿变换之后的特征点云
        ...
        // 获得两个点云的变换矩阵结果，闭环优化得到的当前关键帧与闭环关键帧之间的位姿变换
        ...
        // Add pose constraint
        mtx.lock();
        // 将两帧索引，两帧相对位姿（从当前帧到回环帧的相对位姿）和噪声作为回环约束送入队列
     ...
     * 1、如果是第一帧，用原始imu数据的RPY初始化当前帧位姿（旋转部分）
     * 2、后续帧，用imu里程计计算两帧之间的增量位姿变换，作用于前一帧的激光位姿，得到当前帧激光位姿
     *    也就是把transformTobeMapped从上一帧的 最优位姿估计 转换为当前帧的 最优先验位姿估计
     * 3、为基于优化方式的点云匹配，初始值是非常重要的，一个好的初始值会帮助优化问题快速收敛且避免局部最优解的情况
    */
    void updateInitialGuess()
    {
        // transformTobeMapped是上一帧优化后的最佳位姿，注：这里指lidar的位姿，后面都简写成位姿
        ...
        {
            // 初始的位姿就由磁力计提供，当前帧位姿的旋转部分，用激光帧信息中的RPY（来自imu原始数据）初始化
        ...
        // 用当前帧和前一帧对应的imu里程计 计算相对位姿变换，再用前一帧的位姿与相对变换，计算当前帧的位姿，存transformTobeMapped
        static bool lastImuPreTransAvailable = false;
        static Eigen::Affine3f lastImuPreTransformation;
        // 如果有预积分节点提供的里程记，后端节点给预积分节点提供准确的里程计信息，帮助预积分节点实现紧耦合框架，
        // 然后，预积分节点会反馈给后端框架比较好的位姿先验估计，作为每一帧的初始估计位姿
        if (cloudInfo.odomAvailable == true)
        {
            // 将提供的初值转成eigen的数据结构保存下来
            // 当前帧的初始估计位姿（来自预积分节点，imu里程计在odom坐标系下的位姿）
            ...
            if (lastImuPreTransAvailable == false)
            {
                ...
                // 当前帧相对于前一帧的位姿变换，通过imu里程计提供的位姿计算得到，也就是通过预积分节点得到相对位姿
                Eigen::Affine3f transIncre = lastImuPreTransformation.inverse() * transBack;
                // 前一帧的最优位姿估计
                Eigen::Affine3f transTobe = trans2Affine3f(transformTobeMapped);
                // 当前帧的最优位姿的先验估计，其实就是将这个增量加到上一帧最佳位姿上去，就是当前帧位姿的一个先验估计位姿
                Eigen::Affine3f transFinal = transTobe * transIncre;
                // 将eigen变量转成欧拉角和平移的形式，更新当前帧位姿transformTobeMapped
                pcl::getTranslationAndEulerAngles(transFinal, transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5], 
                                                              transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);

                // 把当前帧预积分提供的位姿保存下来，之后作为上一帧预积分提供的位姿
                lastImuPreTransformation = transBack;
    ...
    */
    void extractNearby()
    {
        ...
        // radiusSearch的输入参数：关键帧位姿，关键帧周围的搜索半径，搜索半径范围内的关键帧索引以及对应的从小到大距离
        ...
        downSizeFilterSurroundingKeyPoses.filter(*surroundingKeyPosesDS); // 下采样后的点云
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            // 如果这个关键帧对应的点云信息已经存储在一个地图容器里，为了减少运算量构建laserCloudMapContainer容器
    ...
    */
    ...
    {
        // 将欧拉角转成eigen的对象，当前帧可靠的先验位姿估计transformTobeMapped
    ...
    */
    void cornerOptimization()
    {
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            // 当前点云坐标系下的点，也就是在当前lidar坐标系下的点
            pointOri = laserCloudCornerLastDS->points[i];
            // 将该点从当前帧通过初始的位姿转换到地图坐标系下去，根据当前帧位姿，变换到世界坐标系（map系）下
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                    ...
                    // 两次叉积，得到点到直线的垂线段单位向量，x分量，下面同理，为了求雅克比方向，也就是梯度下降方向
                    ...
                    coeff.y = s * lb;
                    coeff.z = s * lc;
                    coeff.intensity = s * ld2;  // 距离，也就是残差
    ...
    */
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (planeValid) {
                    ...
                    coeff.y = s * pb;
                    coeff.z = s * pc;
                    coeff.intensity = s * pd2; // 距离，也就是残差
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        for (int i = 0; i < laserCloudSelNum; i++) {
            ...
            matA.at<float>(i, 3) = coeff.z; // 残差关于平移的导数，也就是上面已经计算出来的雅克比矩阵
            matA.at<float>(i, 4) = coeff.x;
            matA.at<float>(i, 5) = coeff.y;
            matB.at<float>(i, 0) = -coeff.intensity;
        }
        // 构造JTJ以及-JTe矩阵
        cv::transpose(matA, matAt); //矩阵转置操作
        matAtA = matAt * matA;
        matAtB = matAt * matB;
        // 求解增量
    ...
    */
    void transformUpdate()
    {
        ...
        if (cloudInfo.imuAvailable == true)
        {
            ...
            {
                double imuWeight = imuRPYWeight; //陀螺仪角度的权重
        ...
        // 最终的结果也可以转成eigen的结构
        incrementalOdometryAffineBack = trans2Affine3f(transformTobeMapped); //得到当前最优位姿估计
    }
    ...
    /** 判断当前帧是否为关键帧
     *  计算当前帧与前一帧位姿变换（相对于前一帧的位姿变换），如果变化太小，不设为关键帧，反之设为关键帧
    */
    bool saveFrame()
    {
        // 如果没有关键帧，那直接认为是关键帧
        ...
        // 旋转和平移量都较小，当前帧不设为关键帧，任何一个旋转大于给定阈值或者平移大于给定阈值就认为是关键帧
    ...
    */
    void saveKeyFramesAndFactor()
    {
        ...
        // 将优化后的位姿更新到transformTobeMapped数组中，作为当前最佳估计位姿
    ...
    void publishOdometry()
    {
        ...
        // Publish TF
        // 发送lidar在odom坐标系下的tf，也就是lidar在odom坐标系下的位姿
